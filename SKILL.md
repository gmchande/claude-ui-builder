...
The helper runs Claude Code with `claude-opus-4-8`, `xhigh` effort by default, JSON output, and an explicit timeout. Builder mode grants edit tools and real shell access; use it only in trusted repos, preferably on a branch or isolated worktree. Evaluator mode is read-only-ish, but `Bash` is still shell access.
...
3. Run builder mode. Let Claude inspect the component system, implement, run checks, start the app when possible, and verify desktop/mobile/browser behavior.
4. Read Claude's handoff. Verify its claims against the real diff, commands, and screenshots.
5. For subjective or high-stakes UI work, run evaluator mode as a separate pass.
6. Use Codex for final technical integration, test review, and follow-up issue creation.
...
`--output-format json` is used internally so the wrapper can parse Claude Code's final result and errors reliably. The human-facing output is still Markdown.
