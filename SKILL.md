...
The helper always launches Claude Code in a visible Zellij session with `--permission-mode bypassPermissions`. If Claude shows its bypass-permissions startup responsibility screen, the helper selects `Yes, I accept` before sending the task. If the Claude prompt never becomes visibly ready, the helper exits instead of pasting into an unknown screen. There is no hidden batch mode, prompt-copy mode, approval-gated mode, or fallback transport. This is intentional: the user can attach, watch stdout, interrupt, and correct Claude directly while Codex remains responsible for planning, integration, and final review.
